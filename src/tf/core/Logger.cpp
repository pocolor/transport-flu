...
*/
    ...
    void Logger::init() {
        ...
        auto gameConsoleSink = std::make_shared<spdlog::sinks::stdout_color_sink_mt>();
        ...
        gameFileSink->set_pattern("[%Y-%m-%d %T] [%l] %v");

        std::vector<spdlog::sink_ptr> gameSinks = { gameConsoleSink, gameFileSink };
        s_gameLogger = std::make_shared<spdlog::logger>(
            "GAME",
            gameSinks.begin(),
            gameSinks.end());

        s_gameLogger->set_level(logLevel);
        s_gameLogger->flush_on(logLevel);
    }
}
